...
# A<-unique(SPREAD$variable2)
# B<-unique(MEAN_SD$variable2)
# 
# setdiff(A,B); setdiff(B,A)
#8SPREAD <- SPREAD %>% dplyr::filter(grepl("Tmean|Tx|Tn|PR|RV", variable2))

#SPREAD <- SPREAD %>% dplyr::filter(grepl("ETP|RV|PR|Tx", variable2))
#SPREAD<- subset(SPREAD, variable2 %in% c("ETP_7","ETP_carre_7","ETP_8","ETP_9","Tmean_7","Tmean_8","Tmean_9","X0.Tx.10_7",'PR_7',"RV_7"))
...
#CLIMATE_NORM$type   <- CLIMATE_NORM$variable  %>%   fct_relabel(~ sub('OBS|y78|y80|y82|y58|y65|y70|y75|.fitted_', '\\1', .x, perl = TRUE))



# DD<- subset(SPREAD, method== "lin_EACH")
# DD<- subset(DD, departement== "AIN")
# DD<- subset(DD, variable== "fitted")
# DD<- subset(DD, year_harvest== "1965")
# #DD <- DD %>% dplyr::filter(grepl("X0.Tx.10", variable))
# DD <- DD %>% dplyr::filter(grepl("Tx.10", variable))
# DD <- DD %>% dplyr::filter(grepl("fitted", variable))

#CLIMATE_NORM$variable3 <- CLIMATE_NORM$variable
...
CLIMATE_NORM2<- data.table::dcast(CLIMATE_NORM, method +departement+type+year_harvest+year_harvescarre_t ~ variable, value.var = "value_norm")
