function_Cross_VALID<- function(TAB){
  ...
  TAB$year_harvescarre_t<-data.frame(scale(TAB$year_harvescarre_t))[,1]
...
model_MCMC  <-function(DAT) {model<-MCMCglmm(log(yield) ~
                                                       ...
                                                       PR_1 +PR_2+PR_3+PR_4+PR_5+PR_6+PR_7+PR_8+
                                                       ETP_1 +ETP_2+ETP_3+ETP_4+ETP_5+ETP_6+ETP_7+ETP_8+
                                                       RV_1 +RV_2+RV_3+RV_4+RV_5+RV_6+RV_7+RV_8, random = ~us(1+year_harvest+year_harvescarre_t                                                          ), data=DAT, pr=TRUE)}
### CROSS VALIDATION

DD<- CAL %>%  filter(departement =="AIN")
DD<- CAL %>%  filter(method =="lin_EACH")
DD<- DD %>%  filter(type =="OBS")

MOD3 <- MCMCglmm(log(yield) ~  Tmean_2  + PR_2 , random = ~us(1+year_harvest):departement, data=BD1$myorigdata[[1]], pr=TRUE)


model<-MCMCglmm(log(yield) ~
                  Tmean_1 +Tmean_2,random = ~us(1+year_harvest), data=DD, pr=TRUE)
model<-lmer(log(yield) ~
                  Tmean_1 +(1|year_harvest), data=DD, pr=TRUE)

DATA_obs <- read.csv("VAR_explicatives_OBSERVED_bar_spr_FALSE.csv", header=T, sep=';',dec=',', na.strings = "NA")
head(DATA_obs)
...
# Validation
  ...
  LIST <- BD1                                         %>% 
    full_join(BD2, by = c("method", "departement"))   %>%            # Join VAL and CAL database
...
# Export the predictions
  ...
  for(k in 1 :dim(GRID_2)[1]){
    DB[[k]] <- VAL %>% 
      ...
      filter(departement %in%  GRID_2$Var2[k]) 
    
    MCMC[[k]]       <- DB[[k]]
    MCMC[[k]]$pred  <-predict(LIST$MCMC[[k]], newdata=MCMC[[k]],marginal=NULL)
    MCMC[[k]]$model <- "MCMC"
    MCMC[[k]]       %<>% mutate(ROUND= i) %>% select(method,type,pred,departement,year_harvest, yield,model, ROUND)
    
    
    Pls[[k]]       <- DB[[k]]
    Pls[[k]]$pred  <- predict(LIST$PLS[[k]], newdata =  Pls[[k]], ncomp = min(1,NCOMP[[k]]))[,1]
    Pls[[k]]$model <- "PLS"
    Pls[[k]]       %<>% mutate(ROUND= i) %>% select(method,type,pred,departement,year_harvest, yield,model, ROUND)
