...
###  MODELES PAR DEPARTEMENT cross validation and prediction
  ...
  CROSS_VALID<- function_Cross_VALID(TAB=YIELD_CLIMAT)
              ...
              head(RES)
              #fwrite(QUALI, "barley_QUALI_par_dep.csv")
              
              ggplot(RES) + geom_point(aes(exp(pred), yield, color=factor(ROUND)))+ facet_wrap(type~model, ncol=7, scales="free")+ theme_pubr()+
              ...
              PREDICTIONS<- function_PRED_YIELD_CC(TAB=YIELD_CLIMAT)
              PRED <- PREDICTIONS[[1]]; 
...
###  Cross validation over all departements
  ...
  CROSS_VALID<- function_Cross_VALID_ALL_DEP(TAB=YIELD_CLIMAT)
              ...
              head(QUALI %>% arrange(desc(R2, MSE)), n=10 )
              head(RES)
              ggplot(RES) + geom_point(aes(exp(pred), yield, color=factor(ROUND)))+ facet_wrap(type~method+model, ncol=6, scales="free")+ theme_pubr()+
                geom_abline(aes(intercept=0, slope=1, color= "gray", linetype="2"))
