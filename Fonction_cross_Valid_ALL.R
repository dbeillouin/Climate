function_Cross_VALID_ALL<- function(TAB){
...
# Validation
  ...
  LIST <- BD1                                         %>% 
    full_join(BD2, by = c("departement"))             %>%            # Join VAL and CAL database
...
# Export the predictions
  ...
  TT1[[i]]  <- RES %>% unnest(newdata, new_Lobell)   %>% mutate(model="new_Lobell") %>% 
                       ...
                       select(pred,departement,year_harvest, yield,model, ROUND)
  TT2[[i]]  <- RES %>% unnest(newdata, new_RF)   %>% mutate(model="new_RF") %>%
                       rename(pred=new_RF) %>% mutate(ROUND= i) %>% 
                       select(pred,departement,year_harvest, yield,model, ROUND)
  
  
  GRID_2<-expand.grid(unique(BD2$departement))
  for(k in 1 :dim(GRID_2)[1]){
    DB[[k]] <- VAL %>% 
      filter(departement %in%  GRID_2$Var1[k]) 
    ...
    Pls[[k]]       <- DB[[k]]
    Pls[[k]]$pred  <- data.frame(predict(LIST$PLS[[k]], newdata =  Pls[[k]], ncomp = max(1,NCOMP[[k]])))[,1]
    Pls[[k]]$model <- "PLS"
    Pls[[k]]       %<>% mutate(ROUND= i) %>% select(pred,departement,year_harvest, yield,model, ROUND)
    
    
    Lasso[[k]]       <- DB[[k]]
    Lasso[[k]]$pred  <- predict(LIST$Lasso[[k]], newdata =  Lasso[[k]], s = BD1$Lassocv[[k]]$lambda.min)[,1]
    Lasso[[k]]$model <- "Lasso"
    Lasso[[k]]       %<>% mutate(ROUND= i) %>% select(pred,departement,year_harvest, yield,model, ROUND)
  
    
    Ridge[[k]]       <- DB[[k]]
    Ridge[[k]]$pred  <- predict(LIST$Ridge[[k]], newdata =  Ridge[[k]], s = BD1$Ridgecv[[k]]$lambda.min)[,1]
    Ridge[[k]]$model <- "Ridge"
    Ridge[[k]]       %<>% mutate(ROUND= i) %>% select(pred,departement,year_harvest, yield,model, ROUND)

    Elnet[[k]]       <- DB[[k]]
    Elnet[[k]]$pred  <- predict(LIST$Elnet[[k]], newdata =Elnet[[k]], s = BD1$Elnetcv[[k]]$lambda.min)[,1]
    Elnet[[k]]$model <- "Elnet"
    Elnet[[k]]       %<>% mutate(ROUND= i) %>%select(pred,departement,year_harvest, yield,model, ROUND)
...
RES<-list(RES1, RES2,RES8,RES9,RES10) %>% bind_rows()
