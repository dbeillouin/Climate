...
TAB$departement<-factor(TAB$departement)

for (n in 1: length(unique(TAB$departement))){
  
TAB2<-TAB[TAB$departement %in% levels(TAB$departement)[n],]
TAB2       <- TAB2[,c(LIST_VAR,"yield")]
...
CAL_FOLDn[[n]] <- rbind(rbind(rbind(CAL_FOLD[[1]],CAL_FOLD[[2]]),CAL_FOLD[[3]]),CAL_FOLD[[4]])
VAL_FOLDn[[n]] <- rbind(rbind(rbind(VAL_FOLD[[1]],VAL_FOLD[[2]]),VAL_FOLD[[3]]),VAL_FOLD[[4]])

}

TAB<-  VAL_FOLDn[[1]]
for (n in 2: 19){
  TAB<- rbind(TAB, VAL_FOLDn[[n]])
  }
# Analyse globale 
...
abline(0,1)
gof(VAL_FOLD$pred, exp(VAL_FOLD$yield))
